...
let validationSchema = Yup.object({
  ...
  email: Yup.string()
    ...
    .email("enter valid email"), // لازم يكون فيه @ و صيغة ايميل صحيحة

  phone: Yup.string()
    .required("phone is required")
    .matches(/^01[1250][0-9]{8}$/, "phone is not valid"), // يتحقق من رقم موبايل مصري صحيح

  password: Yup.string()
    .required("password is required")
    .matches(/^[A-Z][a-z0-9]{6,8}$/, "password is not valid"), 
    // يبدأ بحرف كابتل وبعدها من 6 لـ 8 حروف أو أرقام

  rePassword: Yup.string()
    .required("confirm password is required")
    .oneOf([Yup.ref("password")], "Passwords must match"), // لازم يكون مطابق للباسورد
});
